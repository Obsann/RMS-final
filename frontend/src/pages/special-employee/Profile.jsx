...
export default function SpecialEmployeeProfile() {
    ...
    const fetchProfile = async () => {
        try {
            ...
            setForm({
                ...
                // adminNote is deliberately excluded from edit form for non-admins
                emergencyContact: {
    ...
    const displayInitial = (profile?.username || 'S').charAt(0).toUpperCase();
    ...
    return (
        <DashboardLayout>
            <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <div className="px-8 pb-8">
                        <div className="flex items-end justify-between -mt-16 mb-6">
                            <div className="relative">
                                <div className="w-32 h-32 rounded-full border-4 border-white shadow-lg overflow-hidden bg-blue-600 flex items-center justify-center">
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <p className="text-blue-600 font-semibold text-sm uppercase tracking-wide">
                                    Senior Staff · {profile?.jobCategory || 'Operations Management'}
                                </p>
                            </div>
                        </div>
                        ...
                        <ProfileSection title="Employment Details" icon={<Briefcase className="w-4 h-4 text-indigo-600" />}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormRow label="Staff / Employee ID" editMode={editMode}>
                                    <input type="text" value={form.employeeId} onChange={e => setForm(p => ({ ...p, employeeId: e.target.value }))} className={INPUT} placeholder="e.g. RMS-EMP-001" />
                                ...
                                <div className="col-span-1 md:col-span-2">
                                    <FormRow label="Department / Category" editMode={false}>
                                        <div className="flex items-center gap-3 px-4 py-2.5 border border-gray-100 bg-gray-50 rounded-xl text-sm text-gray-900 min-h-[42px]">
                                            <Briefcase className="w-4 h-4 text-gray-400" />
                                            <span>{profile?.jobCategory || 'Operations Management'}</span>
                                            <span className="ml-auto px-2.5 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold capitalize">
                ...
                {/* ── Legal Section ── */}
                <LegalSection role="special-employee" />
            </div>
        </DashboardLayout>
    );
}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div className="flex items-center gap-3">
                    ...
                    <div><h3 className="font-bold text-gray-900">Security</h3><p className="text-xs text-gray-500 mt-0.5">Manage your password</p></div>
                </div>
                <button onClick={() => setShowPwForm(p => !p)} className="px-4 py-2 rounded-lg text-sm font-semibold border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 shadow-sm transition-colors">
            ...
            {showPwForm && (
                <div className="p-8 space-y-5">
                    {['currentPassword', 'newPassword', 'confirm'].map((field) => {
                        const labels = { currentPassword: 'Current Password', newPassword: 'New Password', confirm: 'Confirm Password' };
                    ...
                    <button onClick={handleChangePassword} disabled={savingPw}
                        className="px-6 py-2.5 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700 disabled:opacity-60 transition-colors flex items-center gap-2 shadow-sm w-full max-w-md">
...
function LegalSection({ role }) {
    ...
    const sections = [
        ...
        { key: 'privacy', icon: Database, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100', title: 'Data Privacy & Personal Data Protection', content: ['All resident and employee personal data processed through this system — including names, addresses, identification numbers, household details, and contact information — is protected under applicable data protection legislation.', 'You are obligated to process personal data only for the specific purpose for which access was granted. Purpose limitation is a legal requirement, not a guideline.', 'Data minimization principles apply: access only the information you need to fulfill your specific task. Browsing or querying records without a legitimate work purpose constitutes a privacy violation.', 'Any accidental or intentional disclosure of personal data to unauthorized parties must be reported to the System Administrator within 24 hours of discovery.', 'Exporting, copying, printing, or transmitting system data outside of approved workflows is prohibited without written authorization from the administrator.'] },
        { key: 'confidentiality', icon: Lock, color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-100', title: 'Confidentiality Obligations', content: ['All information encountered through your use of this system — including resident complaints, escalation records, administrative decisions, digital ID records, and inter-departmental communications — is strictly confidential.', 'Confidentiality obligations survive the termination of your employment or engagement. You remain bound by these obligations after your access is revoked or your role changes.', 'Discussing, sharing, or publishing confidential system information on any platform — including social media, personal devices, or unofficial channels — is a violation of both organizational policy and applicable law.', 'Do not discuss case details, resident identities, or complaint outcomes with unauthorized persons, including family members or unofficial contacts.'] },
        { key: 'security', icon: Shield, color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100', title: 'Security Responsibilities', content: ['You are personally responsible for the security of your login credentials. Use a strong, unique password and update it regularly. Do not use the same password across multiple systems.', 'Never leave an active system session unattended. Log out or lock your session whenever you leave your workstation, even briefly.', 'Report any suspected unauthorized access, phishing attempt, or system anomaly to the administrator immediately — do not attempt to investigate on your own.', 'Do not install unauthorized software, browser extensions, or scripts on any device used to access this system.', role === 'admin' ? 'As System Administrator, you have a heightened duty to maintain system integrity. Any changes to user roles, permissions, or system configuration must be documented and authorized.' : 'Notify the System Administrator if you notice access to functions or data beyond your designated role — this may indicate a misconfiguration that needs prompt correction.'] },
        { key: 'acceptable_use', icon: ClipboardCheck, color: 'text-teal-600', bg: 'bg-teal-50', border: 'border-teal-100', title: 'Acceptable Use Policy', content: ['System resources — including computing capacity, data storage, and network access — may be used exclusively for authorized administrative functions of the Resident Management System.', 'Attempting to circumvent security controls, access audit logs for personal reasons, manipulate historical records, or inject unauthorized data is strictly prohibited and constitutes a criminal offense.', 'Use of system access for commercial purposes, personal gain, or unauthorized research is prohibited.', 'All system interactions are subject to logging. System logs are considered official administrative records and cannot be tampered with or deleted.', 'The system must not be used to discriminate against any resident or employee on the basis of race, religion, gender, ethnicity, disability, or political opinion.'] },
        { key: 'conduct', icon: BookOpen, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100', title: 'Professional Code of Conduct', content: ['All staff accessing this system are expected to maintain the highest standards of professional integrity, impartiality, and ethical conduct in all system interactions.', 'Resident requests and complaints must be processed fairly, promptly, and without bias. Deliberately delaying, ignoring, or misrouting a request is a conduct violation.', 'Communication within the system — including escalation notes, response messages, and broadcast announcements — must remain professional, factual, and free of personal opinion.', 'Retaliation against a resident or colleague for submitting a complaint or escalating a concern is strictly prohibited and may result in immediate suspension of system access.', `${role === 'admin' ? 'As the System Administrator, you set the standard for system governance. Favoritism, data manipulation for personal interest, or selective enforcement of policies is a serious breach of your fiduciary duty.' : 'Report any observed misconduct — including by supervisors — through the proper escalation channel within the system.'}`] },
        { key: 'audit', icon: FileText, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-100', title: 'Audit & Monitoring Consent', content: ['By accessing this system, you explicitly consent to the monitoring, logging, and periodic auditing of all system activities associated with your account.', 'Audit logs capture login history, data queries, record modifications, notification sends, and role changes. These logs are retained for a minimum of 24 months.', 'Audit records may be used in disciplinary proceedings, internal investigations, legal proceedings, or regulatory compliance reviews.', 'You have the right to request a copy of your own activity log from the System Administrator. Requests to delete or alter audit records will not be honored.', role === 'admin' ? 'Administrator actions are subject to an elevated audit standard. All configuration changes, user role modifications, and mass data operations require justification notes and are reviewed quarterly.' : 'Unusual patterns in your access history — such as bulk record queries or off-hours logins — may trigger an automatic security review.'] },
        { key: 'legal', icon: Scale, color: 'text-gray-700', bg: 'bg-gray-50', border: 'border-gray-200', title: 'Legal Basis & Consequences of Violation', content: ['This system operates in compliance with the laws and regulations governing municipal administration, civil service, and personal data protection applicable in the Federal Democratic Republic of Ethiopia.', 'Violations of these policies may result in: formal disciplinary action, suspension or permanent revocation of system access, termination of employment or contract, civil liability, and/or criminal prosecution under applicable law.', 'The organization reserves the right to seek damages, issue injunctions, or pursue any other legal remedy available for violations that result in harm to residents, employees, or the organization.', 'Ignorance of these policies is not a valid defense. By using this system, you acknowledge that you have read, understood, and agree to be bound by all applicable policies and legal obligations.', 'These policies are reviewed annually. Continued use of the system following any published policy update constitutes acceptance of the revised terms.'] },
    ];
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Click any section to expand your legal obligations as a {ROLE_LABEL}</p>
                </div>
            ...
            <div className="divide-y divide-gray-100">
                {sections.map(({ key, icon: Icon, color, bg, border, title, content }) => (
                    <div key={key}>
                        <button
                            onClick={() => toggle(key)}
                            className="w-full flex items-center gap-4 px-8 py-4 text-left hover:bg-gray-50 transition-colors"
                        >
                            <div className={`p-2 ${bg} ${border} border rounded-lg flex-shrink-0`}>
            ...
            <div className="px-8 py-5 bg-amber-50 border-t border-amber-100 flex items-start gap-3">
                ...
                <p className="text-amber-800 text-xs leading-relaxed">
                    ...
                    By logging in and using this system, you confirm that you have read, understood, and agree to comply with all policies and legal obligations outlined above.
                    This acknowledgment is recorded and timestamped against your account in the system audit log.
                    Last policy review: <span className="font-semibold">April 2026</span>.
                </p>
            </div>
        </div>
    );
