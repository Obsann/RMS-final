...
import { Users, UserCog, AlertTriangle, Briefcase, Loader2, ArrowUpRight, MessageSquare, Clock, ChevronDown, ChevronUp, Calendar } from 'lucide-react';
import StatusBadge from '../../components/ui/StatusBadge';
import { useNavigate } from 'react-router-dom';
import { getReports, getRequests, getJobs } from '../../utils/api';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        setLoading(true);
        const [overviewRes, reqRes, jobsRes] = await Promise.allSettled([
          getReports('/overview'),
          getRequests('limit=6'),
  ...
  const cards = [
    ...
    {
      label: 'Open Issues',
      value: overview?.requests?.openIssues ?? 0,
      sub: `${overview?.requests?.pending ?? 0} pending · ${overview?.requests?.inProgress ?? 0} active`,
    ...
    },
    {
      ...
      bg: overdueJobs.length > 0 ? 'bg-red-500' : 'bg-violet-600',
      path: null,
